...
from app.aplicacion.domain.model.karma import Karma
from app.aplicacion.domain.model.voto import Vote
from app.aplicacion.domain.model.voting_user import ArticleVote, VotingUser
...
class VotingUserRepository(
    ...
    def _get_votes_for_articles(
        ...
        if article_vote_entity is not None:
            article_vote = self._article_entity_to_domain_model(
                article_vote_entity)
            return [article_vote]
        else:
            return []
